...
procedure Test_Gamma is
  ...
  procedure Test(x, pgx: Real) is
    -- pgx is computed by Excel 2013, unless specified.
  ...
  begin
    ...
    Put_Line(
      ...
      Real'Image(lgx) & "; " & Real'Image(lpgx)
    );
...
begin
  ...
  Put_Line("x; Gamma(x); Precalculated Gamma(x); Log(Gamma(x))");
