...
procedure Arenstorf is
  ...
  package LFM is new
              G_Matrices(prec_float, 0.0,1.0, "-", Sqrt,"+","-","*","/");
  use LFM;
...
-- Restricted three body problem
  ...
  function f(x:vector4) return vector4 is
    ...
    D1: prec_float:= sqrt(( x1p**2 + x(2)**2  )**3);
    mup_sur_D1: prec_float:= mup/D1;
    D2: prec_float:= sqrt(( x1m**2 + x(2)**2  )**3);
    mu_sur_D2:  prec_float:= mu /D2;
    begin
      return
        ( x(3),    -- x1'
          x(4),    -- x2'
...
--                        + (it*(spal-1)/niter(meth)) );
      ...
      OutText(Integer'Image(it) & " p");
      MoveRel(0, 2*TextHeight('8'));
      OutText(Integer'Image(ntours(meth)) & " t");
      MoveRel(0, 2*TextHeight('8'));
      OutText(Integer'Image(it/ntours(meth)) & " p/t");
      hdern:= h;
    end if; -- dessine
  end loop; -- meth
