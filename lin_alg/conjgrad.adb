...
package body ConjGrad is
  ...
  procedure Choleski_Tridiagonal
    (diago, diagl : in     Vector;
     d, dl        :    out Vector;
     kminor       :    out Index)
  is
  ...
  begin
      ...
      for i in 2 .. neq loop
        j := i - 1;
        dl (j) := diagl (j) / d (j);
        d (i) := diago (i) - dl (j) * dl (j);
        if d (i) <= Real'Model_Epsilon then
          kminor := i;
          return;
        end if;
        d (i) := Sqrt (d (i));
      end loop;
  end Choleski_Tridiagonal;

  procedure Tridiagonal_Precondition (A : Any_matrix; precd, precl, precu : in out Vector) is
  --  Computes the cholesky or lu decomposition of the tridiagonal part of A

    kminor : Index;
    diago : Vector (precd'Range);
    diagl : Vector (precl'Range);
  begin
    ...
    if Defined_symmetric (A) then
      diago := precd;
      diagl := precl;
      Choleski_Tridiagonal (diago, diagl, precd, precl, kminor);
    else
      raise Constraint_Error;
    end if;

    if kminor > 0 then
      raise Tridiagonal_Singularity;
    end if;
  end Tridiagonal_Precondition;
  ...
  begin
    case precond is
      ...
      when tridiag =>  Tridiagonal_Precondition (A, precd, precl, precu);
    end case;
