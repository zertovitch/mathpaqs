...
procedure Test_Discrete_Random_Simulation is
  ...
  type Pba is array(Integer range <>) of Prob_Value;

  package DRS is new Discrete_Random_Simulation(Prob_Value, Pba); use DRS;
  ...
  procedure Test_CDF_by_mode(F: Pba; mode: Simulation_mode; comment: String) is
  ...
  procedure Test_CDF(F: Pba; comment: String) is
  begin
  ...
  flip_coin: constant Pba := (0.0, 0.5);
  dice_1: constant Pba (1..6) := (0.0, 1.0/6.0, 2.0/6.0, 3.0/6.0, 4.0/6.0, 5.0/6.0);
  dice_other : constant Pba:= To_cumulative((1..6 => 1.0/6.0));

  empiric_a: constant Pba := To_cumulative(
  ...
  dice_with_value_1: constant Pba := (0.0, 1.0/6.0, 2.0/6.0, 3.0/6.0, 4.0/6.0, 5.0/6.0, 1.0);
